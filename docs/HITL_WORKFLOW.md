...
uvicorn api_wrapper:app --host 0.0.0.0 --port 8888 --timeout-keep-alive 300 --timeout-graceful-shutdown 300 --loop asyncio
```
...
```bash
python api_client.py --job-id "your-job-id" --feedback "Please add more examples of practical applications of quantum computing"
```
...
3. **Use Async Mode for Efficiency**: For better performance, use the `--async` flag:
   ```bash
   python api_client.py --topic "Quantum Computing" --mode hitl --webhook http://localhost:8889/webhook --async
   ```
...
2. Start a new job:
   ```bash
   python api_client.py --topic "Artificial Intelligence Ethics" --mode hitl --webhook http://localhost:8889/webhook
   ```
...
4. Provide feedback:
   ```bash
   python api_client.py --job-id "your-job-id" --feedback "Please focus more on the ethical implications for privacy"
   ```
